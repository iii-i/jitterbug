...
(define (tail-call-correctness target)
  ...
  (parameterize ([enable-stack-addr-symopt #f])
    ...
    (define precondition-next-instruction
      ...
      (define result (with-vc (bpf-simulate-tail-call bpf-cpu)))
      (bug-assert (normal? result) #:msg "execution of simulated bpf tail call exited abnormally")
      (define ok (car (result-value result)))
      (define tcall-addr (cdr (result-value result)))

      ; Assume the assertions generated inside bpf tail call.
      (assume (vc-asserts (result-state result)))
      ; Assume the assumptions generated inside bpf tail call.
      (assume (vc-assumes (result-state result)))

      (assume (core:bvaligned? tcall-addr (trunc target-bitwidth function-alignment)))
