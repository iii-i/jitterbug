...
(define (per-insn-correctness code target)
  ...
  (define-symbolic* liveset-list boolean? #:length MAX_BPF_JIT_REG)
  ...
  (define run-log
    (for/all ([insns insns #:exhaustive])
      ...
      (define run-log (run-jitted-code target-pc-start target-cpu insns))
      ...
      (define insns-size (code-size insns))
      (define insns-size-bv
        (if (bv? insns-size)
            insns-size
            (integer->bitvector insns-size (bitvector target-bitwidth))))
      (assume (bveq (make-target-pc (bvadd insn-idx bpf-insn-size))
                    (bvadd (make-target-pc insn-idx) insns-size-bv)))

      ; Special handling for BPF function call
      (when (equal? code '(BPF_JMP BPF_CALL))
        (bug-assert
          (equal? (trunc target-bitwidth bpf-call-addr) (core:gen-cpu-pc target-cpu))
        ...
        (set! run-log (cons run-log (run-jitted-code target-pc-start target-cpu insns))))
  ...
  (list
    ...
    (cons 'bpf-memmgr-trace (hybrid-memmgr-trace (bpf:cpu-memmgr bpf-cpu)))
    (cons 'target-cpu target-cpu)
    (cons 'target-bpf-regs target-bpf-regs)
    (cons 'target-pc-base target-pc-base)
    (cons 'target-pc-start target-pc-start)
    (cons 'final-pc-expected final-pc-expected)
    (cons 'final-pc (core:gen-cpu-pc target-cpu))
    (cons 'run-log run-log)))
